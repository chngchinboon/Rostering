# -*- coding: utf-8 -*-
"""
Created on Wed Feb 15 17:06:43 2017
https://github.com/tgvaughan/BuildRoster/blob/master/buildRoster.py
Usage in ipython console:
    %run buildroster constraints.txt 01/08/17 01/09/17 -c output.csv
for help    
    %run buildroster -h 
@author: Owner
"""
...
class Roster:
    ...
    def getPotentialWorkers(self, day):
        ...
        #random choice with weights - use numpy np.random.choice(a,size=None,replace=True,p=None)        
        #weights should be normalized to sum into 1.
        def normalize(canWorkweights):#Normalize weights
            return canWorkweights/canWorkweights.sum()
        #eg. np.random.choice(canWork,1,replace=False,p=canWorkWeights)
        def costFunc(x):
            return 0.01*x.daysSinceHoliday - x.twoDayHolidaysTotal - 0.1*x.holidaysTotal
        canWork.sort(key=costFunc)#sort ascending costs function
    ...
    def populateRoster(self):
        ...
        #only assigned employee to date. Doesn't not assign which shift.
        ...
        for day in self.dates:
            ...
            workingToday.extend(potentials[:shiftsLeft])#brute force pushes lowest cost function into working today list
